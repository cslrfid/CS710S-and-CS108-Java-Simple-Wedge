package com.csl.cs710awedgeapp.fragments;
...
import static com.csl.cs710awedgeapp.MainActivity.mContext;
...
import com.csl.cs710awedgeapp.BuildConfig;
import com.csl.cs710awedgeapp.CustomPopupWindow;
import com.csl.cs710awedgeapp.DrawerListContent;
import com.csl.cs710awedgeapp.adapters.ReaderListAdapter;
            ...
            if (MainActivity.csLibrary4A.isBleConnected()) {
                ...
                while (true) {
                    ...
                    if (cs108ScanData != null) {
                        ...
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
                            if (ActivityCompat.checkSelfPermission(getActivity().getApplicationContext(), BLUETOOTH_CONNECT) != PackageManager.PERMISSION_GRANTED) continue;
                        ...
                        if (DEBUG) MainActivity.csLibrary4A.appendToLog("runnableStart: scanResultA.device.getType() = " + scanResultA.device.getType() + ". scanResultA.rssi = " + scanResultA.rssi);
                        if (scanResultA.device.getType() == BluetoothDevice.DEVICE_TYPE_LE && (true || scanResultA.rssi < 0)) {
                            ...
                            if (DEBUG) MainActivity.csLibrary4A.appendToLog("runnableStart: deviceScanTask: rssi=" + scanResultA.rssi + ", error type=" + scanResultA.device.getType());
                        }
                        scanWait = 0;
                    } else {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
                    ...
                    } else if ((ActivityCompat.checkSelfPermission(mContext, ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED || ActivityCompat.checkSelfPermission(mContext, ACCESS_COARSE_LOCATION) != PackageManager.PERMISSION_GRANTED)) {
